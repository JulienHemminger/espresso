...
def validate_inputs(start_params: dict, end_params: dict, lerp_step_count: int):
    ...
    assert isinstance(start_params, dict) and isinstance(end_params, dict), "Parameters must be dictionaries."
    assert set(start_params.keys()) == set(end_params.keys()), "Start and end parameter keys do not match."
    ...
    for core_key in ["lx", "ly", "lz", "positions", "charges"]:
        assert core_key in start_params, f"Missing critical system parameter: '{core_key}'"
        
    assert len(start_params["positions"]) == len(start_params["charges"]), "Mismatch between positions and charges count."
    assert len(start_params["positions"]) == len(end_params["positions"]), "Start and end positions count must match."
        ...
        if key == "positions":
            ...
            for i, (p1, p2) in enumerate(zip(v1, v2)):
                ...
                if np.array_equal(p1_arr, p2_arr):
                    label_lines.append(f"  P{i}: [{', '.join(f'{x:.1f}' for x in p1_arr)}]")
                else:
                    label_lines.append(f"  P{i}: [{', '.join(f'{x:.1f}' for x in p1_arr)}] → [{', '.join(f'{x:.1f}' for x in p2_arr)}]")
        elif key == "charges":
...
def run_lerp_plot(
    ...
    end_params: dict, 
    lerp_step_count: int,
    plot_metrics: Dict[Tuple[str, Tuple[Tuple[str, Any], ...]], Callable[[espressomd.System, dict], float]]
):
    """
    ...
    results = {metric_key: [] for metric_key in plot_metrics.keys()}
    ...
    fig.text(0.83, 0.5, get_param_label(start_params, end_params), verticalalignment='center', fontsize=8,
