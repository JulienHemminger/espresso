...
def metric_E_dipole(system, params):
    _, E_dipole, _, _ = get_elc_energy_contribs(
        params["gap_size"],
        params["pw_error"],
        system,
        params["prefactor"],
...
def metric_E_recip(system, params):
    _, _, E_recip, _ = get_elc_energy_contribs(
        params["gap_size"],
        params["pw_error"],
        system,
        params["prefactor"],
...
def metric_analytical(system, params):
    ...
    return get_ewald_energy_2d(
        params,
    )  # ana-custom match, but doesnt return values for edges
