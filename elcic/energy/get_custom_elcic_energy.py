...
def _get_far_field_energy(box, gap_size, pw_error, qs, ps, db, dt):
    ...
    if np.any(m_bot):
        ...
        t1 = l_pq_sum(2*lz + ps[m_bot, 2, None], db * delta) + l_pq_sum(2*lz - ps[m_bot, 2, None], delta)
        term_sum = np.sum(t1, axis=0)
        chi_m2_p = [chi_m2_p[i] + chi_local[i] * term_sum for i in range(4)]
    
    if np.any(m_near_top):
        chi_local = _get_chi_components(fx, fy, f, ps[m_near_top], qs[m_near_top], sign=0)
        # Term 2: q * (db * L(z) + delta * L(2lz - z))
        t2 = l_pq_sum(ps[m_near_top, 2, None], db) + l_pq_sum(2*lz - ps[m_near_top, 2, None], delta)
    ...
    if np.any(m_top):
        ...
        # Eq 4.10 logic
        t1 = l_pq_sum(4*lz - ps[m_top, 2, None], dt * delta) + l_pq_sum(2*lz + ps[m_top, 2, None], delta)
        term_sum = np.sum(t1, axis=0)
        chi_p2_m = [chi_p2_m[i] + chi_local[i] * term_sum for i in range(4)]

    if np.any(m_near_bot):
        chi_local = _get_chi_components(fx, fy, f, ps[m_near_bot], qs[m_near_bot], sign=0)
        t2 = l_pq_sum(2*lz - ps[m_near_bot, 2, None], dt) + l_pq_sum(2*lz + ps[m_near_bot, 2, None], delta)
        term_sum = np.sum(t2, axis=0)
        chi_p2_m = [chi_p2_m[i] + chi_local[i] * term_sum for i in range(4)]
...
def _get_config_energy(system, p_set, q_set, prefactor, accuracy, gap_size, physical_lz):
