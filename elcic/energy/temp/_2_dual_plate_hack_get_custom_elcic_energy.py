...
def get_elcic_energy_contribs(system, params: dict):
    ...
    ps_bot1 = ps_orig[m_bot].copy()
    ps_bot1[:, 2] *= -1  # Reflection at z=0

    ps_top1 = ps_orig[m_top].copy()
    ps_top1[:, 2] = 2 * lz - ps_top1[:, 2]  # Reflection at z=lz

    qs_bot1 = qs_orig[m_bot] * db
    qs_top1 = qs_orig[m_top] * dt
    ...
    has_images = len(qs_bot1) > 0 or len(qs_top1) > 0
    if has_images:
        ps_img = np.vstack([p for p in [ps_bot1, ps_top1] if len(p) > 0])
        qs_img = np.concatenate([q for q in [qs_bot1, qs_top1] if len(q) > 0])
