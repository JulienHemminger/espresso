...
def get_elcic_forces(system, params: dict):
    ...
    # --- PART 2: GENERATE PRIMARY IMAGE CHARGES (L_{-1} and L_{+1}) ---
    
    # L_{-1}: Image charges below the bottom interface (z = 0)
    # Sourced from particles in idx_bot. Mirrored to z' = -z
    if len(idx_bot) > 0 and db != 0.0:
        ps_bot_img = ps[idx_bot].copy()
        ps_bot_img[:, 2] = -ps_bot_img[:, 2]
        qs_bot_img = qs[idx_bot] * db
    else:
        ps_bot_img = np.empty((0, 3))
        qs_bot_img = np.empty((0,))

    # L_{+1}: Image charges above the top interface (z = lz)
    # Sourced from particles in idx_top. Mirrored to z' = 2*lz - z
    if len(idx_top) > 0 and dt != 0.0:
        ps_top_img = ps[idx_top].copy()
        ps_top_img[:, 2] = 2.0 * lz - ps_top_img[:, 2]
        qs_top_img = qs[idx_top] * dt
    else:
        ps_top_img = np.empty((0, 3))
        qs_top_img = np.empty((0,))

    # --- DEBUG: Print Image Charges ---
    print(f"DEBUG: Generated {len(qs_bot_img)} primary image charges in L_-1 (bottom)")
    for i, (q, p) in enumerate(zip(qs_bot_img, ps_bot_img)):
        print(f"  CHARGE L_-1 [{i}]: orig_idx={idx_bot[i]}, q_new={q:+.4f}, pos={p}")

    print(f"DEBUG: Generated {len(qs_top_img)} primary image charges in L_+1 (top)")
    for i, (q, p) in enumerate(zip(qs_top_img, ps_top_img)):
        print(f"  CHARGE L_+1 [{i}]: orig_idx={idx_top[i]}, q_new={q:+.4f}, pos={p}")


    # For now, we just pass the original system exactly as before to guarantee equivalence
    # until we construct the LT temporary system in Step 3.
