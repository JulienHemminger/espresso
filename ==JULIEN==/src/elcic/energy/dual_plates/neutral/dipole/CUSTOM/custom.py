...
def get_elcic_energy(system, params: dict):
    ...
    if np.any(mask_top | mask_mid):
        ...
        e_pm1_top = _get_config_energy(system, ps_p1, qs_p1, pref, eps, lz_full)
        e_lt_top = _get_config_energy(system, ps_total_top, qs_total_top, pref, eps, lz_full)

        e_near_top = 0.5 * (e_lt_top - e_pm1_top)
    ...
    if np.any(mask_bot | mask_mid):
        ...
        e_pm1_bot = _get_config_energy(system, ps_m1, qs_m1, pref, eps, lz_full)
        e_lt_bot = _get_config_energy(system, ps_total_bot, qs_total_bot, pref, eps, lz_full)

        e_near_bot = 0.5 * (e_lt_bot - e_pm1_bot)
    
    # Total near-field contribution
    e_near = e_near_top + e_near_bot + 0.5*e_l0
