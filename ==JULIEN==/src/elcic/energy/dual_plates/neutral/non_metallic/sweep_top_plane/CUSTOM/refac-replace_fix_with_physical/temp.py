...
def get_elcic_energy(system, params: dict):
    """
    Computes electrostatic energy for 2D+h system with dielectric interfaces.
    """
    ...
    # Divide particles into three groups based on z-position
    # λ (lambda) is the threshold distance from interfaces
    lambda_threshold = gap  # or use a separate parameter if needed
    ...
    # 1. Base energy (original configuration)
    e_l0 = _get_config_energy(system, ps_orig, qs_orig, pref, eps, gap, lz_full)

    e_near_top = 0.0
    e_near_bot = 0.0

    # ---- TOP contribution (only for particles in L0,+1 and L0,0) ----
    if np.any(mask_top | mask_mid):
        ps_top_source = ps_orig[mask_top | mask_mid]
        qs_top_source = qs_orig[mask_top | mask_mid]
        
        # Create top interface images
        ps_p1 = ps_top_source.copy()
        ps_p1[:, 2] = 2 * (lz_full - gap) - ps_p1[:, 2]
        qs_p1 = qs_top_source * dt
    ...
    # ---- BOTTOM contribution (only for particles in L0,-1 and L0,0) ----
    if np.any(mask_bot | mask_mid):
        ps_bot_source = ps_orig[mask_bot | mask_mid]
        qs_bot_source = qs_orig[mask_bot | mask_mid]
        
        # Create bottom interface images
        ps_m1 = ps_bot_source.copy()
        ps_m1[:, 2] = 2 * gap - ps_m1[:, 2]
        qs_m1 = qs_bot_source * db
