...
def get_elc_energy(p3m, gap_size, pw_error, system):
    ...
    # 1. 3D P3M energy calculation
    # Ensure P3M is configured for the neutralizing background if non-neutral
    system.electrostatics.solver = p3m
    e_3d = system.analysis.energy()["total"]

    # 2. Corrected Dipole Term for Non-Neutral Systems (Eq 3.8)
    # Replaces the standard dipole term (2*pi/vol) * mz^2
    e_corr_dipole = 2.0 * np.pi * ux * uy * uz * (xi1 - (lz / 2.0) * xi0)**2 * prefactor

    # 3. Dynamic Reciprocal Space Correction (Far Formula)
    ...
    exp_p, exp_m = np.exp(zs[:, None] * arg_z), np.exp(-zs[:, None] * arg_z)
    ...
    # 4. ELC Correction (Eq 3.10)
    replica_factor = np.exp(-2.0 * np.pi * f * lz) / (1.0 - np.exp(-2.0 * np.pi * f * lz))
    e_corr_recip = -0.5 * ux * uy * np.sum((1.0 / f) * replica_factor * chi_prod)
    
    # Background subtraction term (Eq 3.9)
    e_bg = 2.0 * np.pi * ux * uy * uz * (-xi0 * xi2 + xi0 * xi1 * (1.0/uz) - (1.0/6.0) * lz**2 * xi0**2)
    
    return e_3d + e_corr_dipole + prefactor * (e_corr_recip + e_bg)