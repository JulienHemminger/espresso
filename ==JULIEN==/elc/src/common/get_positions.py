...
def get_rdm_constrained_points(l_x, l_y, l_z, point_count=2, min_distance=1.0, max_distance=None):
    """
    ...
    for i in range(point_count):
        ...
        for _ in range(max_attempts):
            candidate = (random.uniform(0, l_x), 
                         random.uniform(0, l_y), 
                         random.uniform(0, l_z))
...
def get_rdm_constrained_points_np(l_x, l_y, l_z, point_count=2, min_distance=1.0, max_distance=None):
    return [np.array(x) for x in get_rdm_constrained_points(l_x, l_y, l_z, point_count, min_distance, max_distance)]