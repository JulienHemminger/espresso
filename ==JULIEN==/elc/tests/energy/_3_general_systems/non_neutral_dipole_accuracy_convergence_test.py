...
from elc.src.energy.third_party.get_ewald_energy_2d import (
    get_ewald_energy_2d,
)
...
def test_accuracy_convergence(es_system, show_convergence_plot):
    ...
    for pw_err in pw_errors:
        ...
        p3m = espressomd.electrostatics.P3M(
            prefactor=PREFACTOR, accuracy=pw_err, check_neutrality=False
        )
        system.electrostatics.solver = p3m

        prefactor, e_recip, e_3d, e_non_neutral_corr = get_elc_energy_contribs(
            p3m, gap_size, pw_err, system
        )
        ...
        legacy_errors.append(
            abs(get_legacy_elc_energy(p3m, gap_size, pw_err, system) - ana_energy)
        )
