...
class ELC_Test(ut.TestCase):
    ...
    def test_elc(self):
        ...
        # Set box size and 3D periodicity for ELC
        ...
        # Setup ELC extension wrapper and assign solver
        elc_params = elc_param_sets[case].copy()
        elc = espressomd.electrostatics.ELC(actor=p3m, **elc_params)
        self.system.electrostatics.solver = elc

        elc_res = np.array(self.scan())
        ...
        ax1.plot(z, fx, label="$F_x$")
        ax1.plot(z, fy, label="$F_y$")
        ax1.plot(z, fz, label="$F_z$")
        ax1.set_ylabel("Force")
        ax1.set_title(f"ELC Profile Scan (case: {case})")
        ax1.legend()
        ax1.grid(True)

        ax2.plot(z, e_coulomb, color="purple", label="Coulomb Energy")
